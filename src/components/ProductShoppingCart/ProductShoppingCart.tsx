...
import { ActionsProducts, ProductProps } from '../../types';
...
import './ProductShoppingCart.scss';

export const ProductShoppingCart: React.FC<ProductProps & ActionsProducts> = 
({ id, img, title, price, rate, attribute, discount, count }) => {
    ...
    <li className="cart__item">
      <div className="cart__general">
        <div className="cart__img--wrapper">
          <img className="cart__img" src={img} alt="AppleBYZS852I" />
        </div>
        <div className="cart__count">
          <button className="cart__change" onClick={minusButtonHandler}>-</button>
          <span className="cart__num">{count}</span>
          <button className="cart__change" onClick={plusButtonHandler}>+</button>
        </div>
      </div>
      <div className="cart__info">
        <span className="cart__title">{title}</span>
        <span className="cart__price">{discount || price} ₽</span>
      </div>
      <div className="cart__result">
        <button className="cart__delete" onClick={() => { if (editProductCount) editProductCount(id, 0)}}>
